...
@onready var _parameters_string: Label = $MarginContainer/HBoxContainer/HSplitContainer/Action
@onready var _parameters_string_rich: RichTextLabel = $MarginContainer/HBoxContainer/HSplitContainer/ActionRich
...
var resource: ActionResource:
	set (p_resource):
		...
		if resource.pick_object.has("object") and resource.pick_object.object != null:
			...
			if resource.pick_object.has("name"): _category_name.text = resource.pick_object.name
		else:
...
func _select() -> void:
	...
	if !ESUtils.has_item_in_select(self) and !button_pressed:
		ESUtils.selected_items.append({ "number": get_index(), "object": self, "class": "Action" })
